...
const SessionPrompt = {
  ...
  setPromptExtendSessionClickEvent: function () {
    document.querySelector("#session_prompt_continue_btn").onclick = () => {
      this.refreshSession.apply(this, arguments);
    };
  },

  timeoutPrompt: function () {
    console.log(
      `SessionPrompt.timeoutPrompt called after ${settings.sessionTimeout - settings.graceTimeout}`,
    );
    this.graceTimerRef = setInterval(() => {
      this.promptUpdate.apply(this);
    }, SECOND);
  ...
  promptUpdate: function () {
    if (this.counter === 0) {
      ...
      this.counter -= SECOND;
      console.log(
        `SessionPrompt.promptUpdate - ${this.counter / SECOND} seconds left`,
      );
      this.updateTimeLeftOnPrompt(this.counter);
    }
  },
  ...
  expireSession: function () {
    location.href =
      location.protocol + "//" + location.host + "/respond/session/expired";
  },

  refreshSession: function () {
    clearInterval(this.graceTimerRef);
    const that = this;
    fetch("/respond/session/touch", { credentials: "include" }).then(
      (response) => {
        if (!response.ok) {
          return;
        }
