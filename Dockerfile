...
RUN apt-get install -y shared-mime-info && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
...
RUN bash -lc "DB_ADAPTOR=nulldb.2 bundle exec rake assets:precompile non_digest_assets RAILS_ENV=production SECRET_KEY_BASE=ijustdontcareyoureonlydoingaraketask"
CMD ["./run.sh"]
