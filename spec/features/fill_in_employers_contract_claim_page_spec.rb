...
RSpec.feature "Fill in Employers Contract Claim Page", js: true do
  ...
  scenario "correctly will enable user to check answers and return to edit them" do
    ...
    expect(employers_contract_claim_page.make_employer_contract_claim_question.no.has_checked_field?).to be true if user.make_employer_contract_claim == "No"
    expect(employers_contract_claim_page.make_employer_contract_claim_question.yes.has_checked_field?).to be true if user.make_employer_contract_claim == "Yes"
