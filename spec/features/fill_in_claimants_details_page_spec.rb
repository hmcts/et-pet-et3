...
RSpec.feature "Fill in Claimants Details Page", js: true do
  ...
  scenario "correctly will enable user to check answers and return to edit them" do
    ...
    expect(claimants_details_page.agree_with_early_conciliation_details_question.disagree_conciliation_reason.text).to eql @claimant.disagree_conciliation_reason
    # expect(claimants_details_page.agree_with_employment_dates_question.get).to eql @claimant.agree_with_employment_dates
    expect(claimants_details_page.agree_with_employment_dates_question.employment_start.assert_date_for(@claimant)).to be true if @claimant.agree_with_employment_dates == 'No'
    expect(claimants_details_page.agree_with_employment_dates_question.employment_end.assert_date_for(@claimant)).to be true if @claimant.agree_with_employment_dates == 'No'
